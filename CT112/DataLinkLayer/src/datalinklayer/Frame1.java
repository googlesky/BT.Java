...
public class Frame1 {
    ...
    public Frame1(String str, int Flag) {
        this(str, 65, 27, Flag);
    }

    public Frame1(int dataInBin[][], int[] LRC) {
        this.dataInBin = dataInBin.clone();
        this.dataInBinlast = this.datalast = dataInBin.length;
        this.LRC = LRC.clone();
    }

    public Frame1(String str, int len, int ESC, int Flag) {
        StringGoc = str;
        this.ESC = ESC;
        this.Flag = Flag;
        this.len = len;
        ...
        for (int i = 0; i < str.length(); i++) {
            ...
            if (str.charAt(i) == ESC || str.charAt(i) == Flag) {
                ...
                for (int j = 1; j < rawData.length; j++) {
                    rData[j - 1] = Integer.parseInt(String.valueOf(rawData[j]));
                    if (rData[j - 1] == 1) {
                        if (LRC[j - 1] == 0) {// đếm LRC
                            LRC[j - 1] = 1;
                        } else {
                            LRC[j - 1] = 0;
                        }
                ...
                if (Integer.bitCount(ESC) % 2 == 0) { //trả về true nếu số bit 1 là chẵn
                    rData[0] = 0;
                } else {
                    rData[0] = 1;
                    if (LRC[0] == 0) {
